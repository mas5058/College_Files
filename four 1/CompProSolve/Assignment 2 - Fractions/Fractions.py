...
def rounding(d, gnum, gdenum):
    n = Decimal(gnum/gdenum)
    if d == 'up':
        return ceil(n)
    elif d == 'down':
        return floor(n)
def intpower(num, denum)
    n = num/denum
    return('%e' % n)

def main():
    m_numerator = int(input("Enter Numerator\n"))
    m_denominator = 1
    while True:
        m_denominator = int(input("Enter Denominator\n"))
        if m_denominator == 0:
    ...
    float_comparison()
    print('Rounding up:')
    print(rounding('up',m_numerator, m_denominator))
    print('Rounding down:')
    print(rounding('down',m_numerator, m_denominator))
    print('integer power:')
